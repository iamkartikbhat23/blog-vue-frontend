<template>
    <div v-for="post in posts" :key="post.id" class="border border-gray-300 shadow-md shadow-purple-100 rounded-md p-5 space-y-2 hover:shadow-lg hover:shadow-purple-200 cursor-pointer"  >
        <div class="flex flex-col justify-between" :to="{ name: 'post', params: { slug: post.slug }}">
            <RouterLink class="space-y-2 h-[25rem] flex flex-col justify-between"  :to="{ name: 'post', params: { slug: post.slug }}">
                <div class="text-xl text-gray-800 font-semibold line-clamp-1 capitalize h-[8%]">{{ post.title }}</div>
                <img loading="lazy" class="w-full h-auto object-contain rounded-md " :src="post.image" />
                <div class="text-lg text-gray-700 line-clamp-4 font-normal h-[29%] md:h-[31%]">{{ post.body }}</div>
            ...
            <div class="flex justify-between items-end text-gray-600 pt-4">
                <div v-if="postsStore.isCurrentBlogsAuthor" class="flex gap-1">
